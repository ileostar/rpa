...
     * getElementWindow参数的xpath，Aibote Tool应当使用正常模式下获取的XPATH路径，不要 “勾选java窗口” 复选按钮。对话框子窗口，需要获取对应的窗口句柄操作
    ...
    */
     ...
     * @return {Promise.<boolean>} 成功返回true 失败返回false
    */
    async hidPress(androidId, x, y){
        let strData = this.setSendData("hidPress", androidId, x, y);
        let byteRet = await this.sendData(strData);
        let strRet = byteRet.toString();
        if(strRet == "false")
            return false;
     ...
     * @return {Promise.<boolean>} 成功返回true 失败返回false
    */
    async hidMove(androidId, x, y, duration){
        let strData = this.setSendData("hidMove", androidId, x, y, duration);
        let byteRet = await this.sendData(strData);
        let strRet = byteRet.toString();
        if(strRet == "false")
            return false;
     ...
     * @return {Promise.<boolean>} 成功返回true 失败返回false
    */
    async hidRelease(androidId){
        let strData = this.setSendData("hidRelease", androidId);
        let byteRet = await this.sendData(strData);
        let strRet = byteRet.toString();
        if(strRet == "false")
            return false;
     ...
     * @return {Promise.<boolean>} 成功返回true 失败返回false
    */
    async hidClick(androidId, x, y){
        let strData = this.setSendData("hidClick", androidId, x, y);
        let byteRet = await this.sendData(strData);
        let strRet = byteRet.toString();
        if(strRet == "false")
            return false;
     ...
     * @return {Promise.<boolean>} 成功返回true 失败返回false
    */
    async hidDoubleClick(androidId, x, y){
        let strData = this.setSendData("hidDoubleClick", androidId, x, y);
        let byteRet = await this.sendData(strData);
        let strRet = byteRet.toString();
        if(strRet == "false")
            return false;
     ...
     * @return {Promise.<boolean>} 成功返回true 失败返回false
    */
    async hidLongClick(androidId, x, y, duration){
        let strData = this.setSendData("hidLongClick", androidId, x, y, duration);
        let byteRet = await this.sendData(strData);
        let strRet = byteRet.toString();
        if(strRet == "false")
            return false;
     ...
     * @return {Promise.<boolean>} 成功返回true 失败返回false
    */
    async hidSwipe(androidId, startX, startY, endX, endY, duration){
        let strData = this.setSendData("hidSwipe", androidId, startX, startY, endX, endY, duration);
        let byteRet = await this.sendData(strData);
        let strRet = byteRet.toString();
        if(strRet == "false")
            return false;
    ...
    * @return {Promise.<boolean>} 成功返回true 失败返回false
    */
    async hidDispatchGesture(androidId, gesturePath, duration){
        let strGesturePath = "";
        ...
        let strData = this.setSendData("hidDispatchGesture", androidId, strGesturePath, duration);
        let byteRet = await this.sendData(strData);
        let strRet = byteRet.toString();
        if(strRet == "false")
            return false;
    ...
    * @return {Promise.<boolean>} 成功返回true 失败返回false
    */
    async hidDispatchGestures(androidId, gesturePaths){
        let strGesturesPath = "";
        ...
        let strData = this.setSendData("hidDispatchGestures", androidId, strGesturesPath);
        let byteRet = await this.sendData(strData);
        let strRet = byteRet.toString();
        if(strRet == "false")
            return false;
